package com.vertere.bookingservice.config;  //which folder/namespace this class belongs to
...
import com.vertere.bookingservice.security.JwtAuthFilter;   //our custom filter that reads and verifies JWTs
 ...
 *   missing, requires authentication for every request (unlike
 *   listing-service, booking-service has no public GET routes), and
 *   plugs in jwtAuthFilter to do that authentication check.
 */
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(auth -> auth
                        .anyRequest().authenticated()   //every route requires a valid token - you must be logged in to book anything
                )
